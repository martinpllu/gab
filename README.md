...
Define two or more agents — each with their own persona — point them at a scenario, and watch what emerges. Pure client-side SPA: your OpenRouter key, your scenarios, your transcripts all live in the browser.

## What you can do

- Create "gabs" (scenarios) with a shared scenario prompt that every agent sees
...
- Its own utterances appear as `role: assistant`
- Other agents' utterances appear as `role: user`, prefixed with `[Name] ...`
...
2. The scenario prompt
3. The agent's persona prompt

Agent names are snapshotted into each utterance at write time, so renaming an agent later doesn't shift historical messages. Within a single agent's view, the prefix grows append-only and byte-identical — which is the property OpenRouter's prompt caching needs.

## Turn scheduling

Two partitions of the agent list:

- **Main agents** — `!afterEach`, ordered (drag to reorder, or shuffle per cycle if randomize is on)
...
Each gab has a default model; each agent can override it. Edit [`src/types.ts`](src/types.ts) to add more.
...
├── types.ts                    — Scenario, Agent, Utterance, Model
...
- **Cache hit rate**: the message shape *enables* per-agent prompt caching, but actual hit rate depends on the provider's minimum-prefix-length threshold. Short conversations may not cache.
